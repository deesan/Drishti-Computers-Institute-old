...
Public Class FrmCourseMaterialRecord

    Private Sub FrmCourseMaterialRecord_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FillSession()
        FillCenterCode()
        Getdata()
    End Sub
    ...
    Private Sub cmbSessionSearch1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbSessionSearch1.SelectedIndexChanged
        Try
            Dim A As String = "Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(BRFH) As [Book Receive From HO], RTRIM(BGFC) As [Book Given From Center] ,RTRIM(Date) As [Date] from CourseMaterial where Session = '" & cmbSessionSearch1.Text & "' Order by StudentID"
            Dim B As String = "CourseMaterial"
            Dim C As DataGridView
            C = dgw
            Data(A, B, C)
    ...
    Private Sub cmbRFHO_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbRFHO.SelectedIndexChanged
        Try
            Dim A As String = "Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(BRFH) As [Book Receive From HO], RTRIM(BGFC) As [Book Given From Center] ,RTRIM(Date) As [Date] from CourseMaterial where BRFH = '" & cmbRFHO.Text & "' and Session = '" & lblSession.Text & "' order by StudentID"
            Dim B As String = "CourseMaterial"
            Dim C As DataGridView
            C = dgw
            Data(A, B, C)
    ...
    Private Sub cmbGiven_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbGiven.SelectedIndexChanged
        Try
            Dim A As String = "Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(BRFH) As [Book Receive From HO], RTRIM(BGFC) As [Book Given From Center] ,RTRIM(Date) As [Date] from CourseMaterial where BGFC = '" & cmbGiven.Text & "' and Session = '" & lblSession.Text & "' order by StudentID"
            Dim B As String = "CourseMaterial"
            Dim C As DataGridView
            C = dgw
            Data(A, B, C)
    ...
    Private Sub lblsession_textchanged(sender As Object, e As EventArgs) Handles lblSession.TabIndexChanged
        Reset()
        Getdata()
    End Sub
    ...
    Private Sub Search2Btn_Click(sender As Object, e As EventArgs) Handles Search2Btn.Click
        ...
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(BRFH) As [Book Receive From HO], RTRIM(BGFC) As [Book Given From Center] ,RTRIM(Date) As [Date] from CourseMaterial where CenterCode = @d1 and Session = @d2 order by StudentID", con)
            cmd.Parameters.AddWithValue("@d1", cmbCenterCodeSearch.Text)
            cmd.Parameters.AddWithValue("@d2", cmbSessionSearch1.Text)
            adp = New SqlDataAdapter(cmd)
            ds = New DataSet()
            adp.Fill(ds, "CourseMaterial")
            dgw.DataSource = ds.Tables("CourseMaterial").DefaultView
