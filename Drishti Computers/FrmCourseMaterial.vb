...
Public Class FrmCourseMaterial

    Private Sub FrmCourseMaterial_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FillSession()
        Getdata()
        dtpDate.Text = Today
    ...
    Sub Reset()
        ...
        txtCourse.Text = ""
        txtSession.Text = ""
        cmbBRFH.SelectedIndex = -1
        cmbBGEC.SelectedIndex = -1
        dtpdate.Text = Today
    ...
    Sub Getdata()
        Try
            Dim A As String = "Select RTRIM(StudentID) As [Student ID],RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(BRFH) As [Book Receive From HO], RTRIM(BGFC) As [Book Given From Center] ,RTRIM(Date) As [Date] from CourseMaterial"    '  where Session = '" & lblSession.Text & "'
            Dim B As String = "CourseMaterial"
    ...
    Sub DeleteRecord()
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cq As String = "Update CourseMaterial set BRFH = @d1, BGFC = @d2 where StudentID = @d3"
            ...
            If RowsAffected > 0 Then
                ...
                Else
                    LogFunc(lblUserID.Text, lblUserName.Text, "Deleted the Book Issued details '" & txtStudentname.Text & "' has id '" & txtStudentID.Text & "'")
                    MessageBox.Show("Successfully deleted.", "Record", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
    ...
    Private Sub txtStudentIDSearch_TextChanged(sender As Object, e As EventArgs) Handles txtStudentIDSearch.TextChanged
        Try
            Dim A As String = "Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(BRFH) As [Book Receive From HO], RTRIM(BGFC) As [Book Given From Center] ,RTRIM(Date) As [Date] from CourseMaterial where StudentID like'" & txtStudentIDSearch.Text & "%' order by StudentID"      ' and Session = '" & lblSession.Text & "'
            Dim B As String = "CourseMaterial"
            Dim C As DataGridView
            C = dgw
            Data(A, B, C)
    ...
    '    Try
    '        Dim A As String = "Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(BRFH) As [Book Receive From HO], RTRIM(BGFC) As [Book Given From Center] ,RTRIM(Date) As [Date] from CourseMaterial where Course like'" & txtCourseSearch.Text & "%' and Session = '" & lblSession.Text & "'order by StudentID"
    '        Dim B As String = "CourseMaterial"
    '        Dim C As DataGridView
    '        C = dgw
    '        Data(A, B, C)
    ...
    '    Try
    '        Dim A As String = "Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(BRFH) As [Book Receive From HO], RTRIM(BGFC) As [Book Given From Center] ,RTRIM(Date) As [Date] from CourseMaterial where Session = '" & cmbSessionSearch.Text & "' order by StudentID"
    '        Dim B As String = "CourseMaterial"
    '        Dim C As DataGridView
    '        C = dgw
    '        Data(A, B, C)
    ...
    'Private Sub lblSession_TextChanged(sender As Object, e As EventArgs)
    '    Reset()
    '    Getdata()
    'End Sub
    ...
    Private Sub SaveBtn_Click(sender As Object, e As EventArgs) Handles SaveBtn.Click
        Try
            ...
            Else
                con = New SqlConnection(cs)
                con.Open()
                Dim cb1 As String = "Update CourseMaterial set BRFH = @d1, BGFC = @d2, Date = @d3  where StudentID = @d4"
                cmd = New SqlCommand(cb1)
                cmd.Connection = con
                cmd.Parameters.AddWithValue("@d1", cmbBRFH.Text)
                cmd.Parameters.AddWithValue("@d2", cmbBGEC.Text)
                ...
                Else
                    LogFunc(lblUserID.Text, lblUserName.Text, "Update the student Books Details '" & txtStudentname.Text & "' has ID '" & txtStudentID.Text & "'")
                    MessageBox.Show("Successfully Updated.", "Record", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
    ...
    Private Sub searchbtn1_Click(sender As Object, e As EventArgs) Handles searchbtn1.Click
        ...
        If txtStudentIDSearch.Text <> "" Then
            Try
                Dim A As String = "Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(BRFH) As [Book Receive From HO], RTRIM(BGFC) As [Book Given From Center] ,RTRIM(Date) As [Date] from CourseMaterial where Course like'" & txtCourseSearch.Text & "%' and Session = '" & cmbSessionSearch.Text & "' order by StudentID"      ' and Session = '" & lblSession.Text & "'
                Dim B As String = "CourseMaterial"
                Dim C As DataGridView
                C = dgw
                Data(A, B, C)
    ...
    Private Sub dgw_MouseClick(sender As Object, e As MouseEventArgs) Handles dgw.MouseClick
        Try
            ...
            cmbBRFH.Text = row.Cells(4).Value.ToString
            cmbBGEC.Text = row.Cells(5).Value.ToString
