...
Public Class FrmAdmissioncancel

    Private Sub FrmAdmissioncancel_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Getdata()
        dtpdate.Text = Today
    End Sub
    ...
    Sub Reset()
        ...
        AdmissincancelBtn.Enabled = False
        Getdata()
    End Sub

    Sub Getdata()
        Try
            Dim A As String = "Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(FeesRefound) As [Fees Refound], RTRIM(Reason) As [Reason], RTRIM(Date) As [Date], RTRIM(Status) As [Status] from Admission_Cancel"  'where Session = '" & lblSession.Text & "'
            Dim B As String = "Admission_Cancel"
    ...
    '------------------ Text & Label - Text Change Section Start --------------

    Private Sub txtStudentNameSearch_TextChanged(sender As Object, e As EventArgs) Handles txtStudentNameSearch.TextChanged
        Try
            Dim A As String = "Select RTRIM(StudentID) As [Student ID], RTRIM(Name) As [Student Name], RTRIM(Course) As [Course], RTRIM(Session) As [Session], RTRIM(FeesRefound) As [Fees Refound],RTRIM(Reason) As [Reason], RTRIM(Date) As [Date], RTRIM(Status) As [Status] from Admission_Cancel where Name like'" & txtStudentNameSearch.Text & "%' order by Name"    ' and Session = '" & lblSession.Text & "'
            Dim B As String = "Admission_Cancel"
            Dim C As DataGridView
            C = dgw
            Data(A, B, C)
    ...
    '------------------ Text & Label - Text Change Section End ------------------
